...
for n in range(1, 7):
    ...
    # Короткий хеш содержимого: он попадает в адрес картинки, чтобы после замены файла браузер и PWA не показывали старую копию.
    ver = hashlib.sha1(open(f"apps/web/public/img/islet/islet-{n}.webp", "rb").read()).hexdigest()[:8]
    out.append((n, sm, ver))
ts = "/** Контуры нарисованных островков (`apps/web/public/img/islet/islet-N.webp`): радиус края острова по 72 углам от центра картинки,\n * в долях половины стороны картинки. Снято по прозрачности скриптом `scripts/islet-shapes.py`; при замене картинок пересчитать. */\nexport const ISLET_SHAPE_N = 72;\nexport const ISLET_IMAGES: ReadonlyArray<{ img: number; ver: string; shape: number[]; maxR: number }> = [\n"
for n, sm, ver in out:
    ts += "  { img: %d, ver: \"%s\", maxR: %s, shape: [%s] },\n" % (n, ver, max(sm), ", ".join(str(v) for v in sm))
ts += "];\n"
