...
interface GameDto { id: string; name: string; status: string; teamCount: number; settings: { nodeCount?: number; equidistantStarts?: boolean; maxStartDistanceDiff?: number } }
...
export function SettingsBlock({ game, onSaved }: { game: GameDto; onSaved: () => void }) {
  ...
  async function save(e: FormEvent) {
    ...
    try {
      await api(`/api/games/${game.id}`, { method: "PATCH", body: JSON.stringify({ name, teamCount, settings: { nodeCount, equidistantStarts: equidistant, maxStartDistanceDiff: maxDiff } }) });
  ...
  return (
    <div className="card">
      ...
      {!open && <p className="muted">Команд: {game.teamCount} · узлов: {game.settings.nodeCount ?? 250} · старты: {game.settings.equidistantStarts ? "равноудалённые" : "случайные"} · разница до первого города ≤ {game.settings.maxStartDistanceDiff ?? 3}</p>}
      {open && (
        <form onSubmit={save}>
