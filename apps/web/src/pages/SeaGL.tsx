 ...
 * ходят по кругу, границы ячеек перетекают), координаты искажены плавным шумом (линии изогнутые), блики гаснут
 * пятнами по маске, крупная зыбь темнит воду; при отдалении мелкая каустика уступает крупной. ~30 кадров в
...
float caustic(vec2 p, float t, float w){ vec2 i = floor(p), f = fract(p); float f1 = 8.0, f2 = 8.0;
  for (int y = -1; y <= 1; y++) for (int x = -1; x <= 1; x++) { vec2 g = vec2(float(x), float(y)); vec2 o = hash2(i + g); o = 0.5 + 0.42 * sin(t + 6.2831 * o);
...
void main(){
  ...
  // Искажение координат двумя шумами разной частоты: границы ячеек становятся кривыми и рваными, а не прямыми.
  vec2 warp = (vec2(fbm(wp * 0.008 + vec2(3.1, 7.7)), fbm(wp * 0.008 + vec2(9.2, 1.3))) - 0.5) * 2.4
            + (vec2(fbm(wp * 0.025 + vec2(1.7, 4.4)), fbm(wp * 0.025 + vec2(6.6, 2.9))) - 0.5) * 0.9;
  // Рябь мелкая в единицах карты (ячейка ≈ гекс и меньше); при отдалении она тает — с высоты воду видно гладкой.
  float lod = smoothstep(0.35, 1.3, uK);
  vec2 p1 = wp / 24.0 + warp;
  vec2 p2 = mat2(0.8, -0.6, 0.6, 0.8) * wp / 40.0 + warp * 0.7 + vec2(50.0);
  float c1 = caustic(p1, uT * 0.30, 0.20), c2 = caustic(p2, uT * 0.22 + 2.0, 0.22);
  ...
  float m1 = smoothstep(0.25, 0.8, fbm(wp * 0.006 + uT * 0.006));
  float swell = fbm(wp * 0.0035 + vec2(uT * 0.004, -uT * 0.003));
  vec3 deep = vec3(0.15, 0.40, 0.55), mid = vec3(0.25, 0.54, 0.67);
  vec3 col = mix(deep, mid, swell);
  float light = lod * cau * (0.25 + 0.75 * m1) * 0.42;
