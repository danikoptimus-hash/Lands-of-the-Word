  ...
  return (
    <div className="card">
      <div className="card-head">
        <h2>{t("Битвы")} <span className={"badge" + (pending ? " accent" : "")}>{pending ? t("{n} на проверке", { n: pending }) : active.length}</span></h2>
      ...
      {shown.length === 0 ? <p className="muted">{t("Битв нет.")}</p> : shown.map((b) => (
        <div key={b.id} className={"battle admin " + b.status.toLowerCase()}>
          ...
          <div className="muted" style={{ fontSize: ".9rem" }}>
            {t("ставка {n} ст.", { n: b.bid })}{b.passage ? ` · ${t("отрывок атаки {ref}", { ref: b.passage.ref })}` : ""} · {t("атака: выучено {a}, одобрено {b}", { a: b.attackSum, b: b.attackApproved })}{b.attackDoneAt ? t(" · отправлена") : ""}
            {b.attackDeadline && b.status === "ATTACK" ? ` · ${t("до {d}", { d: new Date(b.attackDeadline).toLocaleString(getLocale()) })}` : ""}
            {b.defensePassage ? ` · ${t("отрывок обороны {ref}", { ref: b.defensePassage.ref })}` : ""}
            {b.status === "DEFENSE" || b.defenseSum > 0 ? ` · ${t("оборона: выучено {a}, одобрено {b} (нужно {c})", { a: b.defenseSum, b: b.defenseApproved, c: b.attackApproved })}${b.defenseDoneAt ? t(" · отправлена") : ""}` : ""}
            {b.defenseDeadline ? ` · ${t("оборона до {d}", { d: new Date(b.defenseDeadline).toLocaleString(getLocale()) })}` : ""}
            {b.defenseBid != null && b.status === "REPELLED" ? ` · ${t("отражено {n} ст.", { n: b.defenseBid })}` : ""}
          </div>
          {b.entries.length > 0 && (
            <ul className="list">
              {b.entries.map((e) => (
                <li key={e.id} style={{ alignItems: "flex-start" }}>
                  <div className="main">
                    <div><span className="badge">{e.side === "ATTACK" ? t("атака") : t("оборона")}</span> <strong>{e.ref}</strong> <span className="muted">· {t("{n} ст.", { n: e.verses })} · {e.nickname} · {new Date(e.createdAt).toLocaleString(getLocale())}</span></div>
