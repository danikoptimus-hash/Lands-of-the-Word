...
## 3a. Правило последовательной генерации

Генератору всегда даём **одну картинку за раз**. Если нужно несколько, в сообщении перечисляем список и просим: «сгенерируй по порядку, по одной, после каждой остановись и жди моего "ок" или правок». Так стиль не расползается и легко откатить неудачную.

Готовая фраза для генератора (после контекста и STYLE):

> Generate the following assets **one at a time, in the listed order**. After each image, stop and wait for my "ok" or corrections before starting the next one. Apply the STYLE block to every image without changing it; only the SUBJECT changes. Asset 1: … Asset 2: … Asset 3: …

## 3b. Утверждённые эталоны (2026-09-08)

✅ Пробная партия принята. Генератор — **ChatGPT**, прозрачный фон умеет. Эталоны лежат в `assets/reference/`:

| Файл | Роль эталона | Замечания |
|---|---|---|
| `walled_city.png` | Эталон для **всех городов**: ракурс ~40°, изолированный объект на прозрачном фоне, «островок» земли под объектом с мягким краем | Идеально. Держим этот ракурс для всех типов городов. |
| `terrain_desert.png` | Эталон для **всех текстур местности**: строго сверху, квадрат | Чуть ярче палитры — приглушу в коде. Бесшовность проверю на файле (2×2). |
| `start_babylon.png` | Эталон для **стартовых локаций**: сцена-«диорама» с фоном (горизонт, закат) и фигурным краем | Для локаций допускаем фон и закатный свет; для городов — нет. |

При генерации следующих партий **прикладывай соответствующий эталон** к сообщению и добавляй в промпт: «in exactly the same style, angle and rendering as the attached reference image».

## 3c. Принято (2026-09-09)
